#!/bin/sh
# acp-verify.sh — verify the AgentCollab bundle: files ↔ manifest ↔ signature.
#
...
#
# Usage:
#   acp-verify.sh                 full L2 verification
#   acp-verify.sh --no-sig        L1 only (integrity, no authenticity)
#   acp-verify.sh --signer <id>   verify against a specific principal
