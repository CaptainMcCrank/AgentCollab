#!/bin/sh
# acp-id.sh — compute the AgentCollab protocol ID from local bundle bytes.
#
...
#
# Usage:
#   acp-id.sh            print the protocol ID (from the manifest's file list)
#   acp-id.sh --root     print the full 64-hex bundle root
#   acp-id.sh --write    regenerate PROTOCOL_MANIFEST.yaml from local files
...
case "${1:-}" in
    ...
    '')
        ...
        ;;
    *)
        die "unknown option: $1 (usage: acp-id.sh [--root|--write])"
        ;;
esac
